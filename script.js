...
function syncLyricsWithAudio() {
    ...
    while (
      ...
      lyricsWithTiming[currentIndex + 1].time <= currentTime
    ) {
      currentIndex++;
    }
    lyricsElement.textContent = lyricsWithTiming[currentIndex].lyric;
  ...
  function goToNextLyric() {
    ...
    // Ganti lirik sesuai dengan waktu baru
    let nextLyricTime = Math.floor(audioElement.currentTime);

    // Temukan indeks lirik berikutnya yang sesuai dengan waktu berikutnya
    while (
      currentIndex < lyricsWithTiming.length - 1 &&
      lyricsWithTiming[currentIndex].time <= nextLyricTime
    ) {
      currentIndex++;
    }
